...
def table_mobo(data): #this is used to create the Motherboard table in tkinter
    ...
    try:
        ...
        window=Tk()
    window.title("Motherboard Table")
    Label(window,fg="blue",text="S. No.",relief='ridge',borderwidth=5,font="none 12 bold").grid(row=0,column=0)
...
def table_storage(data): #this is used to create the Storage table in tkinter
    ...
    try:
        ...
        window=Tk()
    window.title("Storage Device Table")
    Label(window,fg="blue",text="S. No.",relief='ridge',borderwidth=5,font="none 12 bold").grid(row=0,column=0)
    ...
    if uc==1:
        while True:
            ...
            if uc==4:            
                ...
                while True:
                    ...
                    uc=int(input(">"))
                    if uc==1:
                        cur.execute(f'select {column_names} from {table_name} where ram_type="{mobo_data[7]}";')
                        data=cur.fetchall()
                        table_ram(data)
                    elif uc==2:
                        cur.execute(f"select {column_names} from {table_name} {where_command} {additional};")
        ...
        while True:
            ...
            if uc==1:
                ...
                print("Name: ",mobo_data[1],"\nSocket: ",mobo_data[3],"\nChipset: ",mobo_data[4],"\nForm Factor: ",mobo_data[5],"\nMaximum Memory (Ram) Supported: ",mobo_data[6],"GB\nRAM Type (DDR4/DDR5): ",mobo_data[7],"\nMaximum Ram Speed: ",mobo_data[8],"MT/s\nNumber of Ram slots: ",mobo_data[9],"\nPCIE Slots: ",mobo_data[10],"\nPrice: ₹",mobo_data[11],"\nLink: ",mobo_data[12])
            elif uc==4:
                i=0
                while i<len(Ram_data):
                    try:    
                        if Ram_data[i]==Ram_data[i+1]:
                            print(f"Ram Slot {i+1} and {i+2}: {Ram_data[i][1]}\nSize: {Ram_data[i][2]}\nSpeed: {Ram_data[i][3]} MT/s\nRam Type: {Ram_data[i][6]}\nPrice: ₹{Ram_data[i][7]}\nLink: {Ram_data[i][8]}")
                            i+=2
                    except:
                        print(f"Ram Slot {i+1}: {Ram_data[i][1]}\nSize: {Ram_data[i][2]}\nSpeed: {Ram_data[i][3]} MT/s\nRam Type: {Ram_data[i][6]}\nPrice: ₹{Ram_data[i][7]}\nLink: {Ram_data[i][8]}")
                        i+=1
                    else:
                        print(f"Ram Slot {i+1}: {Ram_data[i][1]}\nSize: {Ram_data[i][2]}\nSpeed: {Ram_data[i][3]} MT/s\nRam Type: {Ram_data[i][6]}\nPrice: ₹{Ram_data[i][7]}\nLink: {Ram_data[i][8]}")
                        i+=1
            elif uc==5:
                print("Name: ",gpu_data[1],"\nBase Clock: ",gpu_data[3],"MHz\nBoost Clock: ",gpu_data[4],"MHz\nVRAM: ",gpu_data[5],"GB\nPower Consumed (TDP): ",gpu_data[6],"Watts\nPrice: ₹",gpu_data[7],"\nLink: ",gpu_data[8])
            elif uc==6:
                print("Name: ",storage_data[1],"\nSize: ",storage_data[3],"GB\nType: ",storage_data[4],"\nSpeed: ",storage_data[5],"MB/s\nPrice: ₹",storage_data[6],"\nLink: ",storage_data[7])
            elif uc==7:
                total_ram_price=0
                for i in range(len(Ram_data)):
                    total_ram_price+=Ram_data[i][7]/Ram_data[i][5]
                Total_price=cpu_data[11]+mobo_data[11]+gpu_data[7]+storage_data[6]+total_ram_price
                print("Total Price: ₹",total_ram_price)
            elif uc==8:
                break
    elif uc==5:
        #Username is root 
